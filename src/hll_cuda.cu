...
__global__ void Calculate_HLL_Fluxes_CUDA(Real *dev_bounds_L, Real *dev_bounds_R, Real *dev_flux, int nx, int ny, int nz, int n_ghost, Real gamma, Real *dev_etah, int dir, int n_fields)
{
  // get a thread index
  ...
  if (xid < nx && yid < ny && zid < nz) 
  {
    ...
    f_mz_l = mzl*vxl;
    f_E_l  = (El + pl)*vxl;
    // f_E_l  = (El)*vxl; //No pressure on E advection
    #ifdef DE
    f_ge_l = dgel*vxl;
    #endif
    #ifdef SCALAR
    ...
    f_mz_r = mzr*vxr;
    f_E_r  = (Er + pr)*vxr;
    // f_E_r  = (Er )*vxr; //No pressure on E advection
    #ifdef DE
    f_ge_r = dger*vxr;
    #endif
    #ifdef SCALAR
